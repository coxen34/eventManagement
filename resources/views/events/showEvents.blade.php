...
<div class="flex justify-center">
     <h2 class="font-bold text-lg bg-black text-black p-1 w-1/2 text-center rounded"style="background-color: rgba(251, 251, 239, 0.6);">Todos los Eventos</h2>
</div>
   <br>
    @if (session('success'))
        <div>
        ...
        @foreach ($events->chunk(4) as $eventRow)
            <ul class="flex flex-wrap justify-between -my-4">
                @foreach ($eventRow as $event)
                    <li class="border p-4 mb-4 w-64 min-h-[12rem] flex flex-col justify-between "style="background-color: rgba(251, 251, 239, 0.6);">
                        <div>
                        ...
                        <div class="flex space-x-24">
                            ...
                            <form method="POST" action="{{ route('events.destroy', ['eventId' => $event->id]) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="delete-event font-bold text-red-500 bg-red-100 hover:bg-red-200 rounded">Eliminar</button>
                            </form>
                        </div>
                    </li>
                @endforeach
